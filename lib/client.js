					...
					this.workletNode.port.onmessage = (e) => {
						...
						try {
							...
							if (rms > 200) {
								...
								// 静音：从录音开始或最后出声起算，静音满 1.5 秒自动停止
								const anchor = self.lastSpeechTime > 0 ? self.lastSpeechTime : self.startTime;
								if (anchor > 0 && now - anchor >= 1500 && self.recording) {
									// 连续静音 1.5 秒 → 自动停止
			...
			const startListening = useCallback(() => {
				...
				const asr = new BridgeASR({
					...
					onResult: (text, isFinal) => {
						...
						if (isFinal) {
							// 最终结果：一次性在光标位置插入
							// 去重：若新 final 是已有内容的子串或相同，跳过（bridge 可能补发重复句）
							const existing = stateRef.current.final;
							const t = String(text || "").trim();
							if (t !== "" && existing.indexOf(t) === -1) {
								stateRef.current.final = existing === "" ? t : existing + t;
							}
